...
export const changePasswordUser  = async (req, res) => {
    try {
        ...
        const userUpdate = await User.findByIdAndUpdate(user._id, { password: ciphertext,ShouldResetPassword:false,verified:true }, { new: true });
        await passwordChangedEmailUser(userUpdate)
...
// password changes function User

const passwordChangedEmailUser = async (user)=>{
...
const generatedPassword = async (user)=>{
    ...
    let info = await transporter.sendMail({
        ...
        html: `Hello user`+ user.firstName + `` + user.lastName+ ` your password is ` + originalPassword + ` which will need to be changed upon login you can access the system on`+ "<a>www.booking.com</a>", // html body
    });
